%macro report_new(internal=Y);
	...
	ods pdf text="^S={font_size=24pt}"; /* spacer */
	ods graphics on/width=850px height=250px noborder;
	...
	proc sgplot data=current_data noautolegend;
		hbar work_status_today/group=work_status_today datalabel  DATALABELFITPOLICY=NONE datalabelattrs=(size=16px);
	...
	proc sgpanel data=current_data noautolegend;
		...
		hbar work_status_today / group=work_status_today groupdisplay=cluster datalabel  DATALABELFITPOLICY=NONE datalabelattrs=(size=16px);
	...
	ods graphics on/width=850px height=400px noborder;

	proc sgpanel data=current_data noautolegend;
		panelby lob_area tribe /novarname;
		hbar work_status_today / group=work_status_today groupdisplay=cluster DATALABELFITPOLICY=NONE datalabel;
		colaxis label="N of staff";
	run;
	...
	ods graphics on/width=850px height=300px noborder;
	ods pdf text="^S={font_size=&breakspace.}"; /* spacer */
	ods pdf text="^S={font_size=16pt font_weight=bold just=center}Work Schedule of Next Day - All Staff of GAC Guangzhou";

	proc sgplot data=current_data noautolegend;
		hbar work_status_tomorrow/group=work_status_tomorrow datalabel DATALABELFITPOLICY=NONE  datalabelattrs=(size=16px);
		xaxis label="N of Staff";
		yaxis label=" ";
	run;
	...
	proc sgpanel data=current_data noautolegend;
		panelby lob_area /novarname columns=1;
		hbar work_status_tomorrow / group=work_status_tomorrow groupdisplay=cluster datalabel DATALABELFITPOLICY=NONE datalabelattrs=(size=16px);
		colaxis label="N of staff";
		rowaxis  valueattrs=(size=16px);
	run;
	...
	proc sgpanel data=current_data noautolegend;
		panelby lob_area tribe /novarname;
		hbar work_status_tomorrow / group=work_status_tomorrow groupdisplay=cluster DATALABELFITPOLICY=NONE datalabel;
		colaxis label="N of staff";
	run;
	...
	proc sgplot data=current_data noautolegend;
		vbar location_region/group=location_region datalabel DATALABELFITPOLICY=NONE  datalabelattrs=(size=16px);
		xaxis label="Staff Location";
		yaxis label="N of Staff";
	run;

	ods graphics off;
	footnote "Based on IP address";
	...
	%if  &internal.=Y %then
		%do;
			...
			proc sgplot data=current_data noautolegend;
				hbar Health_status/group=Health_status datalabel  DATALABELFITPOLICY=NONE datalabelattrs=(size=16px);
				xaxis label="N of Staff";
				yaxis label=" ";
			run;
